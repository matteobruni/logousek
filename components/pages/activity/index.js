...
const Activity = () => {
  ...
  const getActivityFromConf = useCallback(() => Object.values(games).reduce(
    (result, gamesInType) => {
  ...
  const GetPointsForTask = useCallback(() => {
    ...
    const actualDifficulty = difficultyList.find(difficulty => difficulty.id == activityDifficulty)
    console.log("actualDifficulty", actualDifficulty, activityDifficulty, difficultyList, actualDifficulty?.points)
  ...
  useEffect(() => {
    if (gameState === "finish") {
      const redirectToGameMenu = () => {
        ...
        setUserPoints((GetPointsForTask() || DEFAULT_POINTS_FOR_TASK) * correctTasks);
      };
